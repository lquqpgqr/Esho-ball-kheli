...
void iDraw()
{
	...
	else if (gamestate==6) top();
	else if (gamestate == 8) {
		singlePlayer(); goalkeeperJump(); 
		ballKick(); player();
			if(face>9) celebration();  

	}
	else if (gamestate==9) multiPlayer();
...
void iMouse(int button, int state, int mx, int my)
{
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		if (gamestate == 0 && mx>b1[3].x && mx<b1[3].x + 322 && my>b1[3].y && my<b1[3].y + 510) gamestate = 1;//menu1 to menu2
		...
		else if (gamestate == 0 && mx>b1[5].x && mx<b1[5].x + butLen && my>b1[5].y && my<b1[5].y + butWid) gamestate = 6;//menu1 to top
			...
			gamestate = 1; playerRun = 0; kick = false; ballx = 0; bally = 0; postCheck = 0; gk4 = 0; ballShow = 0; countCount[0] = 0; countCount[1] = 0, face = 0; 
		}//single to menu2
		else if(gamestate==9 && mx>butx7 && mx<butx7+butLen && my>buty7 && my<buty7+butWid) gamestate=1;//multiplayer to menu2
...
int main()
{
	...
	rr=iSetTimer(100,run);
	if(playerRun>43){
		iPauseTimer(rr);printf("testing1");}
	bb = iSetTimer(30,ballMove);
	iSetTimer(100,goalkeeMove);
