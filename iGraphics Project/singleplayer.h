...
int whoPlays = 0, currentPlayer = 0,face=0;
char dispScore[2][30];
...
void goalkeeMove()
{
	if (playerRun > 33 && gk4 < 13)
	{
		...
		if (gk4 == 12){ 
			//printf("ll%dll\n", goalCount[0]);
			playerRun = 0; kick = false; ballx = 0; bally = 0; postCheck = 0;  gk4 = 0;
			if (face%2==1)
				currentPlayer = 0;
...
void goalkeeperJump()
{
	iShowImage(0,0,dispLen,dispWid,gkplayer[value][gk4]);
}
...
void player(){
	...
	iShowImage(0, 0, dispLen, dispWid, play[currentPlayer][playerRun]);
}
...
void ballKick(){
	...
	iSetColor(0, 0, 0);
	iText(1140, 395, dispScore[0], GLUT_BITMAP_TIMES_ROMAN_24);
	iText(1140, 375, dispScore[1], GLUT_BITMAP_TIMES_ROMAN_24);
...
void isGoal(){
	...
	if ((postCheck == 1 || postCheck == 2) && value == 0){
		if (gk4==12)ballShow = 1;
		else ballShow = 0;
	}
	else if ((postCheck == 4 || postCheck == 5) && value == 1){
		if (gk4 == 12)ballShow = 1;
		else ballShow = 0;
	}
	else if (postCheck == 2 && value == 2){
		if (gk4 == 12)ballShow = 1;
		else ballShow = 0;
	}
